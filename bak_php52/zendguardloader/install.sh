...
if [ ! -f "$extFile" ]; then
	...
	if [ ! -f $php_lib/ZendOptimizer-3.3.3-linux-glibc23-x86_64.tar.gz ]; then
	...
	if [ ! -d $php_lib/${LIBNAME}-${LIBV} ]; then
		cd $php_lib
		tar xvf ZendOptimizer-3.3.3-linux-glibc23-x86_64.tar.gz
	fi

	cd $php_lib/ZendOptimizer-3.3.3-linux-glibc23-x86_64
	cp $php_lib/ZendOptimizer-3.3.3-linux-glibc23-x86_64/data/5_2_x_comp/ZendOptimizer.so $extDir/ZendOptimizer.so
fi
