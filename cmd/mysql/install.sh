...
if [ ! -f $MDIR/source/mysql/mysql-${VERSION}-osx10.11-x86_64.tar.gz ]; then
	wget -O $MDIR/source/mysql/mysql-${VERSION}-osx10.11-x86_64.tar.gz https://cdn.mysql.com/archives/mysql-5.6/mysql-${VERSION}-osx10.11-x86_64.tar.gz
fi

if [ ! -d $MDIR/source/mysql/mysql-${VERSION}-osx10.11-x86_64 ]; then
	cd $MDIR/source/mysql && tar -zxvf $MDIR/source/mysql/mysql-${VERSION}-osx10.11-x86_64.tar.gz
fi


if [ ! -d $DIR/mysql ];then
	mkdir -p $DIR/mysql
	cp -r $MDIR/source/mysql/mysql-${VERSION}-osx10.11-x86_64/ $DIR/mysql/
fi
