...
# cd /Applications/mdserver/bin/reinstall/cmd/mysql/mysql56 && bash install.sh
...
if [ ! -f $MDIR/source/mysql/mysql-${VERSION}-macos10.13-x86_64.tar.gz ]; then
	wget -O $MDIR/source/mysql/mysql-${VERSION}-macos10.13-x86_64.tar.gz https://cdn.mysql.com/archives/mysql-5.6/mysql-${VERSION}-macos10.13-x86_64.tar.gz
fi

if [ ! -d $MDIR/source/mysql/mysql-${VERSION}-macos10.13-x86_64 ]; then
	cd $MDIR/source/mysql && tar -zxvf $MDIR/source/mysql/mysql-${VERSION}-macos10.13-x86_64.tar.gz
fi


if [ ! -d $DIR/mysql/mysql${MY_VERSION}/bin ];then
	mkdir -p $DIR/mysql/mysql${MY_VERSION}
	cp -r $MDIR/source/mysql/mysql-${VERSION}-macos10.13-x86_64/ $DIR/mysql/mysql${MY_VERSION}
fi

if [ ! -d $DIR/mysql/mysql${MY_VERSION}/my.cnf ]; then
	mkdir -p $DIR/mysql/mysql${MY_VERSION}
	cp $MDIR/bin/reinstall/tpl/mysql/my56.cnf $DIR/mysql/mysql${MY_VERSION}/my.cnf
fi
